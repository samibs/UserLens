...
class MarkdownGenerator {
    ...
    async generateDocumentation(components, journeys, outputPath) {
    ...
    async generateOverview(components, journeys, outputPath) {
    ...
    async generateGettingStarted(components, journeys, outputPath) {
        ...
        const authComponents = components.filter(c => c.name.toLowerCase().includes('login') ||
        ...
        const navComponents = components.filter(c => c.name.toLowerCase().includes('nav') ||
    ...
    async generateAuthenticationGuide(authComponents, outputPath) {
        const loginComponent = authComponents.find(c => c.name.toLowerCase().includes('login') ||
            c.name.toLowerCase().includes('signin'));
        const registerComponent = authComponents.find(c => c.name.toLowerCase().includes('register') ||
            c.name.toLowerCase().includes('signup'));
    ...
    async generateNavigationGuide(navComponents, outputPath) {
        ...
        const menus = navComponents.filter(c => c.name.toLowerCase().includes('menu') ||
    ...
    async generateFirstStepsGuide(components, journeys, outputPath) {
    ...
    async generateFeatures(components, journeys, outputPath) {
        ...
        for (const [category, categoryComponents] of Object.entries(componentsByCategory)) {
            if (categoryComponents.length > 0) {
                ...
                const significantComponents = categoryComponents.filter(c => c.userActions.length > 0 ||
                    c.props.length > 3 ||
    ...
    async generateCategoryOverview(category, components, outputPath) {
    ...
    async generateComponentDoc(component, outputPath) {
        ...
        if (component.props.length > 0) {
            ...
            const userVisibleProps = component.props.filter(p => !p.name.startsWith('on') &&
                !p.name.includes('ref') &&
                !p.name.includes('key') &&
    ...
    async generateWorkflows(components, outputPath) {
    ...
    async generateWorkflowDoc(workflow, outputPath) {
    ...
    async generateIndex(outputPath) {
    ...
    extractNavigationPurpose(component) {
